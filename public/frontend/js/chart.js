...
closer.chart = function () {
    var chart, layers, tooltip
    ...
    */
    initialize = function ($container) {
        ...
        chart.xscale = d3.scale.linear()
            .domain([closer.firstYear, closer.lastYear])
    ...
    */
    legend = function (layers, parent) {
        ...
        rows.append("td").text(function (d) {
            return d.key + ' (' + d.percentage.toFixed(2) + '% de ' + chart.legendParent.key +')';
        })
    ...
    */
    open = function (data, color) {
        ...
        var data = data || closer.currentNode.data;
        var color = color || closer.currentNode.color;
        ...
        var projected = closer.currentYear - closer.firstYear;
        ...
        overflows.append("rect").attr("width", 10)
            .attr("height", chart.height).attr('x', chart.xscale(closer.lastYear));
    ...
    */
    slideLayers = function (x) {
        ...
        function updateInfo(year) {
            ...
            // boundary year
            var newIndex = Math.max(year - closer.firstYear, 0);
            if (yearIndex === newIndex) return;
            yearIndex = newIndex;
            closer.cards.update(closer.currentNode.data);
            legend();
        }
    ...
    */
    drawLayers = function (data) {
        ...
        if(closer.thisYear != chart.year){
            chart.layersWidth = chart.xscale(closer.thisYear);
            chart.year = closer.thisYear;
        }
