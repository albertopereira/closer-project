...
closer.table = function () {
    ...
    */
    var initialize = function ($container, data) {
        ...
        if (data instanceof Array) {
            ...
            tableStats = tables[closer.section];
    ...
    */
    renderNode = function (node, level, container) {
        ...
        // append popover
        if (node.descr !== undefined && node.descr.length !== 0) {
    ...
    */
    renderSparkline = function (node, cell) {
        ...
        sparkline.on('mousemove', function(){
            ...
            .attr("cy", yscale(node.values[year - closer.firstYear].val));
    ...
    */
    renderMaplink = function(data, cell){
        ...
        $(cell).click(function(){
            closer.section = findSection(data.hash).key.toLowerCase();
            switchMode('t', true);
            // give enough time to load data
            setTimeout(function(){
                closer.treemap.open(data.hash, false, false);
            }, 50)
